...
//     if (!item.valor) {
//       console.warn(`Aviso: Item ${item.id} sem valor definido.`);
//       return acc;
//     }
...
//       console.warn(`Erro: Valor inválido no item ${item.id}: "${valorStr}"`);
//       return acc;
//     }
...
//   const total = saldosDoMes.reduce((acc, item) => {
//     if (!item.valor) {
//       console.warn(`Aviso: Item ${item.id} sem valor definido.`);
//       return acc;
//     }
...
/**
 * Lista todos os saldos armazenados.
 * @returns {Promise<Array>} Lista de objetos saldo
 */
async function listar() {
  const jsonValue = await AsyncStorage.getItem('@saldos');
  return jsonValue != null ? JSON.parse(jsonValue) : [];
...
/**
 * Salva um novo saldo na lista.
 * @param {Object} saldo - Objeto saldo para salvar. Deve conter pelo menos 'valor' e 'data'.
 */
async function salvar(saldo) {
 ...
 * @param {number} id - ID do saldo
 * @returns {Promise<Object|undefined>} Objeto saldo ou undefined se não encontrar
 */
async function buscar(id) {
  const saldos = await listar();
  return saldos.find(saldo => saldo.id === id);
 ...
 * @param {number} id - ID do saldo a remover
 */
async function remover(id) {
  const saldos = await listar();
  const novaLista = saldos.filter(saldo => saldo.id !== id);
 ...
 * @param {Object} novoSaldo - Objeto saldo atualizado (deve conter o mesmo id)
 */
async function atualizar(novoSaldo) {
  const saldos = await listar();
  const novaLista = saldos.map(saldo =>
...
/**
 * Soma o valor de uma lista de saldos.
 * @param {Array} saldos - Array de objetos saldo com campo 'valor'
 * @returns {number} Total somado dos valores
 */
...
/**
 * Calcula o total somando todos os saldos armazenados.
 * @returns {Promise<number>} Total somado
 */
async function calcularTotal() {
  const saldos = await listar();
  console.log("Lista de saldos para soma:", saldos);
...
export default {
  ...
  calcularTotal,
  calcularTotalPorMes,
  somarSaldos, // exporta a função de soma pura para teste independente
};
